...
namespace New_Student_Enquiry
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // this combobox has DropDownStyle=DropDownList, user can only pick one the available choices
            cbxDepartment.Items.AddRange(programs.Keys.ToArray());
            cbxDepartment.SelectedIndex = 0; // Select the first choose


            // this combobox has DropDownStyle=DropDown, so user can enter enter their own text
            cbxHowDidYouHear.Items.Add("another student");
            cbxHowDidYouHear.Items.Add("another school");
            cbxHowDidYouHear.Items.Add("another search");

        }

        private void cbxDepartment_SelectedIndexChanged(object sender, EventArgs e)
        {
            LstDegrees.ClearSelected();  // the selection changed event
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            // Beacuse user cam type, whatever they type is considered to be index -1
            ...
            summeryBuilder.Append("Summary");
            summeryBuilder.Append("\n\nDepartment: ");
            summeryBuilder.Append(cbxDepartment.Text);

            summeryBuilder.Append("\n\nPrograms: \n ");

            foreach (object degree in LstDegrees.SelectedItems)
            {
                summeryBuilder.Append(degree);
                summeryBuilder.Append("\n");
            }

            summeryBuilder.Append("\nYou heard about us from: ");
            summeryBuilder.Append(cbxHowDidYouHear.Text);

            string summary = summeryBuilder.ToString();
